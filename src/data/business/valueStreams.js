    ...
    groups: [
      ...
      },
      {
        ...
        items: [
          ...
          p('Percentage rent (retail)', 'Bill turnover-based rent from retail tenants against agreed breakpoints, sharing in the upside of a successful trading location.', 'Configure lease-level breakpoints, natural or lease-defined, with base-plus-top-up or pure-turnover structures', 'Calculate percentage rent in a review-first (edit) mode before posting', 'Post the charge through to billing and reconcile turnover rent'),
          p('Retail sales departments', 'Segment a single tenant\'s turnover by trade category so different product lines, exclusions or co-located brands can each bill on their own terms.', 'Define sales departments for distinct categories, exclusions or split turnover', 'Set a separate breakpoint set per department', 'Rely on lease audit rights, not the system, to verify a tenant\'s declared split'),
          p('Retail sales capture & estimation', 'Get tenant turnover into the system on time, whether reported directly, submitted by the tenant, or estimated when nothing has been received.', 'Capture sales directly, including bulk entry across tenants', 'Offer tenant self-service submission where a separately licensed portal is in place', 'Estimate sales to maintain billing continuity when a tenant has not reported'),
          p('Retail category hierarchy & reporting', 'Classify stores and tenants on a standard three-tier hierarchy and report turnover, trading density and growth against it.', 'Maintain the store category, tenant category and tenant type hierarchy', 'Run standard retail reports for performance ranking, gross sales trends and lease profiles', 'Export sales data to external benchmarking providers'),
      ...
      {
        // The AR-010 to AR-090 retail operating model. The mechanics of
        // calculating and billing turnover rent live in Rent Billing above;
        // this group covers the surrounding business process — how retail deals
        // are structured, how compliance is enforced, and how trading
        // performance is managed.
        title: 'Retail Turnover & Trading Performance',
        note: 'The retail operating model around turnover rent — governance, tenant mix, deal structuring, certificate compliance and trading analytics.',
        items: [
          p('Retail leasing governance', 'Define who structures retail deals, what they can agree without escalation, and how turnover terms are approved — the control framework around a rent line that varies with tenant trade.',
            'Set the retail leasing team structure and decision rights',
            'Define approval thresholds for turnover percentages and bespoke tiered deals',
            'Govern exceptions where a national tenant negotiates away from standard terms'),
          p('Tenant mix, zoning & category management', 'Curate the trade mix across a centre so categories complement rather than cannibalise, and so the offer matches the catchment.',
            'Plan target tenant mix by zone and trade category',
            'Classify stores against the store category, tenant category and tenant type hierarchy',
            'Review mix against catchment and trading-density evidence'),
          p('Retail lease structuring & turnover rent determination', 'Agree the commercial shape of a retail deal — whether rent is pure base, base plus turnover top-up, or pure turnover, and at what percentages and thresholds.',
            'Select the rental structure: pure base, base plus turnover top-up, or pure turnover',
            'Negotiate turnover percentages by trade category and agree tier thresholds',
            'Agree the turnover base per lease — whether turnover is measured against basic rent or a wider gross-rental definition',
            'Structure year-one arrangements that convert to a fixed base in later years'),
          p('Turnover base & exclusions definition', 'Establish, lease by lease, exactly which revenue counts toward turnover — the single parameter that drives every turnover calculation on that lease.',
            'Define the turnover base: basic rent, or gross rental including operating costs, rates and (for restaurants) outside seating',
            'Agree excluded revenue streams such as dispensary or scheduled-medication sales',
            'Record the agreed basis so billing and audit work from the same definition'),
          p('Turnover certificate & audit compliance', 'Enforce the reporting obligations in the lease — monthly submissions, the annual audited certificate, and the true-up that follows.',
            'Monitor monthly turnover submission against the lease deadline, typically the 2nd of the following month with lease-specific variants',
            'Track the audited turnover certificate due within 90 days of each tenant\'s own financial year-end, falling due on a rolling basis',
            'Escalate non-submission as a lease breach through breach letter, remedy period and monetary penalty',
            'Compare the certified figure against monthly reported figures and raise the final debit or credit true-up',
            'Commission an external auditor to verify financial statements and point-of-sale data where lease provisions allow'),
          p('Franchise, guarantor & co-tenancy management', 'Manage the covenant behind a retail lease and the contractual dependencies between tenants in a centre.',
            'Record franchise and guarantor arrangements standing behind the tenant covenant',
            'Capture co-tenancy provisions where a tenant\'s obligations depend on other tenants trading',
            'Link related leases to a master occupant so sales history follows the tenant across renewals and store moves'),
          p('Special tenant category leasing', 'Apply the differing turnover treatments that particular retail formats demand, rather than forcing every unit onto one model.',
            'Treat food-court units so communal seating is excluded and genuinely dedicated seating is included in turnover and trading density',
            'Distinguish a genuine kiosk lease, which carries a turnover component, from very short-term promotional space let at a fixed premium',
            'Handle entertainment and leisure units broadly as standard retail while allowing internal box-office versus catering analysis',
            'Exclude parking from turnover unless defined as gross-rental space, and bill aerial and antenna base stations purely per square metre'),
          p('Trading performance reporting & analytics', 'Turn captured sales into the trading view the asset team manages by — who is performing, who is not, and how the centre is trending.',
            'Rank tenants by performance and produce top and bottom schedules by store, region or category',
            'Compare gross sales year on year and on a rolling twelve-month basis',
            'Report turnover, trading density and growth, drawing on up to twenty-four months of sales history',
            'Export sales data to external benchmarking providers using the retail category alignment'),
          p('Retail renewal & turnover-driven repricing', 'Use proven trading performance to reset the rent at renewal, converting sustained turnover top-up into committed base rent.',
            'Review turnover history and top-up paid across the expiring term',
            'Negotiate a proportion of the top-up into an increased base rent at renewal',
            'Carry sales history into the renewed lease so the calculation has an evidence base'),
        ],
      },
      {
